...
/**
 * Copy of {@link org.apache.poi.ss.formula.functions.LookupUtils}
 * 
...
final class LookupUtils {
		...
		public RowVector(TwoDEval tableArray, int rowIndex) {
			...
			if (rowIndex < 0 || rowIndex > lastRowIx) {
				throw new IllegalArgumentException(
						"Specified row index (" + rowIndex + ") is outside the allowed range (0.." + lastRowIx + ")");
			}
		...
		public ValueEval getItem(int index) {
			if (index > _size) {
				throw new ArrayIndexOutOfBoundsException(
						"Specified index (" + index + ") is outside the allowed range (0.." + (_size - 1) + ")");
			}
	...
	/**
	 * @return <code>null</code> if the supplied area is neither a single row nor a
	 *         single colum
	 */
	...
	/**
	 * Enumeration to support <b>4</b> valued comparison results.
	 * <p>
	 * Excel lookup functions have complex behaviour in the case where the lookup
	 * array has mixed types, and/or is unordered. Contrary to suggestions in some
	 * Excel documentation, there does not appear to be a universal ordering across
	 * types. The binary search algorithm used changes behaviour when the evaluated
	 * 'mid' value has a different type to the lookup value.
	 * <p>
	 *
	 * A simple int might have done the same job, but there is risk in confusion
	 * with the well known <tt>Comparable.compareTo()</tt> and
	 * <tt>Comparator.compare()</tt> which both use a ubiquitous 3 value result
	 * encoding.
	 */
	...
	public interface LookupValueComparer {
		/**
		 * @return one of 4 instances or <tt>CompareResult</tt>: <tt>LESS_THAN</tt>,
		 *         <tt>EQUAL</tt>, <tt>GREATER_THAN</tt> or <tt>TYPE_MISMATCH</tt>
		 */
	...
	/**
	 * Processes the third argument to VLOOKUP, or HLOOKUP (<b>col_index_num</b> or
	 * <b>row_index_num</b> respectively).<br>
	 * Sample behaviour:
	 ...
	 * @throws EvaluationException when the specified arg cannot be coerced to a
	...
	/**
	 * The second argument (table_array) should be an area ref, but can actually be
	 * a cell ref, in which case it is interpreted as a 1x1 area ref. Other scalar
	 * values cause #VALUE! error.
	 */
	public static TwoDEval resolveTableArrayArg(ValueEval eval) throws EvaluationException {
		...
		if (eval instanceof RefEval) {
			...
			// It doesn't matter if eval is a 2D or 3D ref, because that detail is never
			// asked of
			// AreaEval.
	...
	/**
	 * Resolves the last (optional) parameter (<b>range_lookup</b>) to the VLOOKUP
	...
	public static boolean resolveRangeLookupArg(ValueEval rangeLookupArg, int srcCellRow, int srcCellCol)
		...
		if (valEval instanceof StringEval) {
			...
			Boolean b = parseBoolean(stringValue);
			if (b != null) {
	 ...
	 * @param vector         the values to be searched. For VLOOKUP this is the
	 *                       first column of the tableArray. For HLOOKUP this is the
	 *                       first row of the tableArray.
	...
	/**
	 * Encapsulates some standard binary search functionality so the unusual Excel
	 * behaviour can be clearly distinguished.
	 */
	...
	/**
	 * Excel has funny behaviour when the some elements in the search vector are the
	 * wrong type.
	 *
	 */
	...
	/**
	 * Excel seems to handle mismatched types initially by just stepping 'mid' ix
	 * forward to the first compatible value.
	 * 
	 * @param midIx 'mid' index (value which has the wrong type)
	 * @return usually -1, signifying that the BinarySearchIndex has been narrowed
	 *         to the new mid index. Zero or greater signifies that an exact match
	 *         for the lookup value was found
	 */
	...
	/**
	 * Once the binary search has found a single match, (V/H)LOOKUP steps one by one
	 * over subsequent values to choose the last matching item.
	 */
	...
	public static LookupValueComparer createLookupComparer(ValueEval lookupValue, boolean matchExact,
		...
		if (lookupValue == BlankEval.instance) {
			...
			// empty string in the lookup column/row can only be matched by explicit empty
			// string
	...
	/**
	 * Extracted from {@link org.apache.poi.ss.formula.functions.Countif}
	 * 
