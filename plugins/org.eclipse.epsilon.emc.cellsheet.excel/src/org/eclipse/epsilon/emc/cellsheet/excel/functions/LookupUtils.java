...
final class LookupUtils {
   ...
   * types, and/or is unordered. Contrary to suggestions in some Excel documentation, there does not
   * appear to be a universal ordering across types. The binary search algorithm used changes
   * behaviour when the evaluated 'mid' value has a different type to the lookup value.
   * <p>
   *
   * A simple int might have done the same job, but there is risk in confusion with the well known
   * <tt>Comparable.compareTo()</tt> and <tt>Comparator.compare()</tt> which both use a ubiquitous 3
   ...
   */
  public static int resolveRowOrColIndexArg(ValueEval rowColIndexArg, int srcCellRow,
    ...
    try {
      veRowColIndexArg =
          OperandResolver.getSingleValue(rowColIndexArg, srcCellRow, (short) srcCellCol);
  ...
  public static TwoDEval resolveTableArrayArg(ValueEval eval) throws EvaluationException {
    ...
    if (eval instanceof RefEval) {
      ...
      // It doesn't matter if eval is a 2D or 3D ref, because that detail is never asked of
      // AreaEval.
  ...
  /**
   * Resolves the last (optional) parameter (<b>range_lookup</b>) to the VLOOKUP and HLOOKUP
   * functions.
   * 
   * @param rangeLookupArg must not be <code>null</code>
   */
  public static boolean resolveRangeLookupArg(ValueEval rangeLookupArg, int srcCellRow,
      int srcCellCol) throws EvaluationException {
  ...
  public static int lookupIndexOfValue(ValueEval lookupValue, ValueVector vector,
      boolean isRangeLookup) throws EvaluationException {
   ...
   */
  private static int lookupIndexOfExactValue(LookupValueComparer lookupComparer,
      ValueVector vector) {
  ...
  /**
   * Encapsulates some standard binary search functionality so the unusual Excel behaviour can be
  ...
  /**
   * Excel seems to handle mismatched types initially by just stepping 'mid' ix forward to the first
   * compatible value.
   * 
   ...
   */
  private static int handleMidValueTypeMismatch(LookupValueComparer lookupComparer,
      ValueVector vector, BinarySearchIndexes bsi, int midIx) {
    int newMid = midIx;
   ...
   */
  private static int findLastIndexInRunOfEqualValues(LookupValueComparer lookupComparer,
      ValueVector vector, int firstFoundIndex, int maxIx) {
  ...
  public static LookupValueComparer createLookupComparer(ValueEval lookupValue, boolean matchExact,
    ...
    throw new IllegalArgumentException(
        "Bad lookup value type (" + lookupValue.getClass().getName() + ")");
  }
