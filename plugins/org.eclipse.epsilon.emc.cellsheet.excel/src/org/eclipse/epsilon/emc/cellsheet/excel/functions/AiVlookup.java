...
/**
 * Abstract Interpretation of VLOOKUP that returns the cell address of the
 * resolved value.
 * 
...
public class AiVlookup extends AbstractVar3or4ArgAiFunction {
	...
	public ValueEval evaluate(int srcRowIndex, int srcColumnIndex, ValueEval lookup_value, ValueEval table_array,
		...
		try {
			// Evaluation order:
			// lookup_value , table_array, range_lookup, find lookup value, col_index, fetch
			// result
			...
			boolean isRangeLookup = LookupUtilsDelegate.resolveRangeLookupArg(range_lookup, srcRowIndex,
					srcColumnIndex);
			...
			// FIXME: Make this more robust - lets check for additional sheets and resolve
			// the actual
			// value
			...
			return new StringEval(book.getCell(_tableArray.getFirstSheetIndex(), rowIndex, colIndex).getId());

		} catch (EvaluationException e) {
			throw new UnsupportedOperationException("Error occurred during evaluation of " + getAiName(), e);
		}
	}
